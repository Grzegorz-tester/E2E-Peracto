...
Then(/^all "([^"]*)" links should resolve without an error$/, async function (this: ScenarioWorld, elementKey: string) {
    ...
    const containerSelector = getElementLocator(page, elementKey, globalConfig);
    const hrefs = await page.locator(`${containerSelector} a:visible`).evaluateAll((els) =>
        els.map((el) => el.getAttribute("href")).filter((href): href is string => !!href && !href.startsWith("mailto:") && !href.startsWith("tel:")),
    );

    const uniqueHrefs = [...new Set(hrefs)];
    expect(uniqueHrefs.length, `No links found in "${elementKey}" - check the selector/page state`).toBeGreaterThan(0);
