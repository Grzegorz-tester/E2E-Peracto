...
const checkLinksResolve = async (page: ScenarioWorld["screen"]["page"], hrefs: string[]) => {
...
const gatherHrefs = async (page: ScenarioWorld["screen"]["page"], elementIdentifier: string): Promise<string[]> => {
    const hrefs = await page.locator(elementIdentifier).evaluateAll((els) =>
        els
