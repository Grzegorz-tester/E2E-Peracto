...
Then(/^the "([^"]*)" input should be rejected as (empty|invalid)$/, async function (this: ScenarioWorld, elementKey: ElementKey, kind: "empty" | "invalid") {
    const {
        screen: { page },
        globalConfig,
    } = this;
    ...
    const validity = await page.$eval(elementIdentifier, (el: HTMLInputElement) => ({
