...
When(/^I read window\.__NEXT_DATA__$/, async function (this: ScenarioWorld) {
    ...
    const nextData = await page.evaluate(() => (window as unknown as { __NEXT_DATA__: unknown }).__NEXT_DATA__);
...
Then(/^the page data should reference "([^"]*)"$/, async function (this: ScenarioWorld, expectedSubstring: string) {
    ...
    expect(raw).toContain(expectedSubstring);
});
...
Then(/^the page data at "([^"]*)" should be present$/, async function (this: ScenarioWorld, fieldPath: string) {
    const raw = this.globalVariables["__NEXT_DATA__"];
    if (!raw) {
        throw new Error(`No __NEXT_DATA__ captured - "I read window.__NEXT_DATA__" must run first.`);
    }
    ...
    expect(isPresent, `Expected "${fieldPath}" to be present in __NEXT_DATA__, got: ${JSON.stringify(value)}`).toBe(true);
});
...
// first one.
Then(/^every entry in the page data at "([^"]*)" should have a "([^"]*)" field$/, async function (this: ScenarioWorld, arrayPath: string, key: string) {
    const raw = this.globalVariables["__NEXT_DATA__"];
    if (!raw) {
        throw new Error(`No __NEXT_DATA__ captured - "I read window.__NEXT_DATA__" must run first.`);
    }
    ...
    expect(Array.isArray(value), `Expected "${arrayPath}" to be an array, got: ${JSON.stringify(value)}`).toBe(true);
    const entries = value as Record<string, unknown>[];
    expect(entries.length, `Expected "${arrayPath}" to have at least one entry`).toBeGreaterThan(0);

    const missing = entries.filter((entry) => entry?.[key] === undefined);
    expect(missing, `Entries missing "${key}": ${JSON.stringify(missing)}`).toEqual([]);
});
