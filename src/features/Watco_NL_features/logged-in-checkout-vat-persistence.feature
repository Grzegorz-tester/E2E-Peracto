@regression
Feature: Logged-in checkout - VAT number persistence to account (NL)

  # Migrated from P3Playwright watco/tests/basket-checkout/nl/logged-in-checkout-vat-persistence.test.ts
  # NL mirror of the UK suite. Pay on Account is VAT-gated here, but
  ...
  # visible by the time it's needed - no extra handling required. NL
  # zero-rates for ANY valid VAT number, even a domestic NL-to-NL
  # delivery, so the carried-through VAT amount here is zero. Places TWO
  # real orders via Pay on Account against the shared "account test user
  # with vat" account (NL000099998B57 baseline) - one with an edited VAT
  # number, one restoring the original. SIMPLIFIED from the source the
  # same way the UK file is - see that file's docblock for why.
  ...
  Scenario: An edited VAT number persists to the account after the order is placed
    ...
    And the "VAT number" should equal the value "NL000099998B57"

    When I fill in the "VAT number" input field with "NL000099999B12"
    ...
    Then the "VAT number" should equal the value "NL000099999B12"
