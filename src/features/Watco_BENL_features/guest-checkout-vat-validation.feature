@regression
Feature: Guest checkout - VAT number field validation (BE-NL)

  # Migrated from P3Playwright watco/tests/basket-checkout/benl/guest-checkout-vat-validation.test.ts
  # BE-NL mirror of the UK suite, but Pay on Account is VAT-gated here -
  # the first scenario applies a VALID VAT first (revealing Pay on
  # Account), then dirties the field again and uses Pay on Account as
  # normal. The second scenario (fresh session, no valid VAT ever
  # applied) has no Pay on Account to select - it uses "Pay by card" /
  # the Adyen terms checkbox instead, which is always offered.
  ...
  Background:
    ...
    And I fill in the "City" input field with "Brussel"
    And I fill in the "Postcode" input field with "1000"
    And I select the "België" option from the "Country" dropdown
  ...
  Scenario: An invalid VAT number is rejected, then a valid one is accepted and later dirtying it again blocks proceeding
    ...
    Then the "validation message" should equal text "Het ingevoerde btw-nummer is ongeldig. Voer een btw-nummer in met het formaat BE1234567890."
    And the "VAT number" should have class "is-invalid"

    When I fill in the "VAT number" input field with "BE0123456749"
    And I click on the "VAT apply" button
