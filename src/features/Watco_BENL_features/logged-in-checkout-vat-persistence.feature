@regression
Feature: Logged-in checkout - VAT number persistence to account (BE-NL)

  # Migrated from P3Playwright watco/tests/basket-checkout/benl/logged-in-checkout-vat-persistence.test.ts
  # BE-NL mirror of the UK suite. Pay on Account is VAT-gated here, but
  ...
  # visible by the time it's needed - no extra handling required. Places
  # TWO real orders via Pay on Account against the shared "account test
  # user with vat" account (BE0123456749 baseline) - one with an edited
  # VAT number, one restoring the original. SIMPLIFIED from the source
  # the same way the UK file is - see that file's docblock for why.
  ...
  Scenario: An edited VAT number persists to the account after the order is placed
    ...
    And the "VAT number" should equal the value "BE0123456749"
    ...
    When I am on the "account-profile" page
    Then the "VAT number" should equal the value "BE0123456749"
