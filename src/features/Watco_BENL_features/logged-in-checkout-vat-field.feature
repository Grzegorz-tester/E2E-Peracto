@regression
Feature: Logged-in checkout - VAT number field (BE-NL)

  # Migrated from P3Playwright watco/tests/basket-checkout/benl/logged-in-checkout-vat-field.test.ts
  # BE-NL mirror of the UK suite - Pay on Account is VAT-gated here, and
  ...
  Scenario: Has a saved VAT number - field is pre-populated, Pay on Account is visible on load, and both are editable
    ...
    And the "VAT number" should equal the value "BE0987654321"
