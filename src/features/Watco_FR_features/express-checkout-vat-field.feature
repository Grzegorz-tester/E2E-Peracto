@regression
Feature: Express Checkout - VAT number field (FR)

  # Migrated from P3Playwright watco/tests/basket-checkout/fr/express-checkout-vat-field.test.ts
  # FR mirror of the UK suite - same deferred scope (real wallet-sheet
  ...
  Scenario: VAT field is visible above the wallet buttons, and Pay on Account is not offered
    ...
    And the "express VAT number" should have attribute "placeholder" with value "FRXX123456789"
  ...
  Scenario: An invalid VAT number is rejected, then a valid one is applied cleanly
    ...
    And I click on the "Checkout now" button
    And I click on the "express checkout toggle" element
    And I fill in the "express VAT number" input field with "FRA1234567"
    And I click on the "express VAT apply" button
    Then the "express validation message" should equal text "Le numéro de TVA entré n'est pas valable. Veuillez entrer un numéro de TVA au format: FRXX123456789."
    And the "express VAT number" should have class "is-invalid"

    When I fill in the "express VAT number" input field with "FRAB123456789"
    And I click on the "express VAT apply" button
    Then the "express VAT number" should not have class "is-invalid"
    And the "express VAT number" should equal the value "FRAB123456789"
