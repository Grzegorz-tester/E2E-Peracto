@regression
Feature: Logged-in checkout - VAT number persistence to account (FR)

  # Migrated from P3Playwright watco/tests/basket-checkout/fr/logged-in-checkout-vat-persistence.test.ts
  # FR mirror of the UK suite. Places TWO real orders via Pay on Account
  # against the shared "account test user with vat" account
  # (FRAB123456789 baseline) - one with an edited VAT number, one
  ...
  Scenario: An edited VAT number persists to the account after the order is placed
    ...
    Then the "VAT number" should equal the value "FRCD987654321"
    ...
    When I am on the "account-profile" page
    Then the "VAT number" should equal the value "FRAB123456789"
