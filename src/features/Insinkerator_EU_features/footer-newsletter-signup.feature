...
Feature: Footer newsletter sign-up
  ...
  # Ported from P3Playwright's insinkerator_eu/tests/footer/
  # footer-newsletter-signup.test.ts. The newsletter form lives in the site
  # footer, present on every page, and is a native <input type="email"
  # required> with no custom client-side validation UI - empty/malformed
  # submissions are asserted via the input's own validity state, not a
