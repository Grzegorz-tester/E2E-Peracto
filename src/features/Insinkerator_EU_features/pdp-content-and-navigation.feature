...
Feature: PDP content and navigation
  ...
  # pdp-content-and-navigation.test.ts, on /products/standard-460 - a
  # configurable-bundle PDP also covered for its configurator/basket
  # behaviour by product-configurator.feature. This covers the rest of that
  # same PDP's content and navigation surface: the Overview/Features/
  # Specifications/Downloads accordion and the separate FAQs accordion
  # (both single-open behaviour), the Comparison Table's "View Product"
  # links, the Product Features carousel, and the image zoom lightbox.
