@regression
Feature: Logged-in checkout - VAT number field (DE)

  # Migrated from P3Playwright watco/tests/basket-checkout/de/logged-in-checkout-vat-field.test.ts
  # DE mirror of the UK suite - Pay on Account is VAT-gated here, and
  ...
  Scenario: Has a saved VAT number - field is pre-populated, Pay on Account is visible on load, and both are editable
    ...
    And the "VAT number" should equal the value "DE987654321"
