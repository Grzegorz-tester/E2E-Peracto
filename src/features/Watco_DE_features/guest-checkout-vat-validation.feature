@regression
Feature: Guest checkout - VAT number field validation (DE)

  # Migrated from P3Playwright watco/tests/basket-checkout/de/guest-checkout-vat-validation.test.ts
  # DE mirror of the UK suite, but Pay on Account is VAT-gated here - the
  ...
  Scenario: An invalid VAT number is rejected, clearing after an invalid entry cleans up the error, then a valid one is accepted and later dirtying it again blocks proceeding
    When I fill in the "VAT number" input field with "DE12"
    And I click on the "VAT apply" button
    Then the "validation message" should equal text "Die eingegebene USt-IdNr. ist ungültig. Bitte geben Sie eine Umsatzsteuer-Identifikationsnummer im Format DE123456789 ein"
    ...
    When I fill in the "VAT number" input field with "DE987654321"
    ...
    Then the "validation message" should equal text "Es liegen nicht gespeicherte Änderungen in diesem Feld vor. Bitte übernehmen Sie die Änderungen oder leeren Sie das Feld, bevor Sie fortfahren"
