@regression
Feature: Logged-in checkout - VAT number field (BE-FR)

  # Migrated from P3Playwright watco/tests/basket-checkout/befr/logged-in-checkout-vat-field.test.ts
  #
  ...
  # guest-checkout-vat-field.feature's own docblock for the live
  # evidence. This mirrors UK/IE/FR's structure as a result: Pay on
  # Account is offered regardless of VAT state, for both a no-VAT and a
  # saved-VAT account.
  ...
  Scenario: Has a saved VAT number - field is pre-populated and editable
    ...
    And the "VAT number" should equal the value "BE0403294259"
