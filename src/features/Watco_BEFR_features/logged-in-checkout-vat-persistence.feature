@regression
@places-real-order
Feature: Logged-in checkout - VAT number persistence to account (BE-FR)

  # Migrated from P3Playwright watco/tests/basket-checkout/befr/logged-in-checkout-vat-persistence.test.ts
  # BE-FR mirror of the UK suite. Pay on Account is always available
  # here (live-verified this session - see guest-checkout-vat-field.
  # feature's own docblock; an earlier note incorrectly claimed it was
  # VAT-gated), so no extra handling is needed either way. Places TWO
  # real orders via Pay on Account against the shared
  # "account test user with vat" account (BE0411905847 baseline) - one
  # with an edited VAT number, one restoring the original. SIMPLIFIED
  ...
  Scenario: An edited VAT number persists to the account after the order is placed
    ...
    And the "VAT number" should equal the value "BE0411905847"
