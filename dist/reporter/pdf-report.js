...
const formatClock = function () {
  let nanoseconds = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 0;
...
const getRunner = () => {
  try {
    const name = (0, _child_process.execSync)('git config user.name', {
      encoding: 'utf-8'
    }).trim();
    if (name) return name;
  } catch {
  ...
  try {
    return _os.default.userInfo().username;
  } catch {
    return '-';
  }
};
...
const renderScenarioPage = (scenario, projectName, featureName, runner) => {
...
const buildDonut = (passed, failed, skipped) => {
  const total = passed + failed + skipped;
  if (total === 0) return `<div class="donut" style="background:#e4e7eb"></div>`;
  const passedPct = passed / total * 100;
  const failedPct = failed / total * 100;
  const stops = [`${STATUS_COLOR.passed} 0% ${passedPct}%`, `${STATUS_COLOR.failed} ${passedPct}% ${passedPct + failedPct}%`, `${STATUS_COLOR.skipped} ${passedPct + failedPct}% 100%`];
  return `<div class="donut" style="background:conic-gradient(${stops.join(', ')})"></div>`;
};
const buildHtml = (features, generatedAt) => {
  ...
  const host = (0, _parseEnv.env)('UI_AUTOMATION_HOST', '');
  const runner = getRunner();
  const totalDurationNs = allScenarios.reduce((sum, el) => sum + sumDurations(el), 0);
  const featureRows = scenariosByFeature.map(_ref => {
    let {
      name,
      scenarios
    } = _ref;
  ...
  const scenarioPages = scenariosByFeature.flatMap(_ref2 => {
    let {
      name,
      scenarios
    } = _ref2;
    return scenarios.map(scenario => renderScenarioPage(scenario, projectName, name, runner));
  }).join('\n');
  return `<!doctype html>
<html>
<head>
...
const run = async () => {
  ...
  const defaultPath = _path.default.join(_path.default.dirname((0, _parseEnv.env)('HTML_REPORT_FILE')), projectName, defaultFilename);
  const outputPath = (0, _parseEnv.env)('PDF_REPORT_FILE', defaultPath);
  _fs.default.mkdirSync(_path.default.dirname(outputPath), {
    recursive: true
  });
