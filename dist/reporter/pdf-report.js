...
const renderStep = step => {
  ...
  const errorBlock = step.result?.error_message ? `<div class="step-error">${escapeHtml(step.result.error_message)}</div>` : '';
...
const buildHtml = features => {
  ...
  const tocItems = scenariosByFeature.map(_ref => {
    let {
      name,
      scenarios
    } = _ref;
  ...
  const featureSections = scenariosByFeature.map(_ref2 => {
    let {
      name,
      uri,
      scenarios
