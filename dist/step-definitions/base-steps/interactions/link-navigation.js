...
(0, _cucumber.When)(/^I click on the "([^"]*)" element and note the response status$/, {
  timeout: 40000
...
(0, _cucumber.Then)(/^the noted response status should be less than (\d+)$/, async function (thresholdText) {
...
(0, _cucumber.Then)(/^a heading with the text "([^"]*)" should be displayed$/, async function (text) {
  const {
    screen: {
      page
    }
...
(0, _cucumber.When)(/^I click on the "([^"]*)" (?:link|element) via its href on this origin$/, async function (elementKey) {
  const {
    screen: {
      page
    },
