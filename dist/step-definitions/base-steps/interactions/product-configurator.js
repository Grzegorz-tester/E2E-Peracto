...
(0, _cucumber.When)(/^I select the first priced configurator option and validate the PDP total updates$/, async function () {
  const {
    screen: {
      page
    },
  ...
  const totalSelector = (0, _webElementHelper.getElementLocator)(page, "configurator total value", globalConfig);
  const optionsSelector = (0, _webElementHelper.getElementLocator)(page, "configurator options", globalConfig);
  ...
  const optionTexts = await options.evaluateAll(els => els.map(el => el.closest("button")?.textContent?.trim() ?? ""));
  ...
  this.globalVariables["configured extra"] = JSON.stringify({
    name,
    priceDelta
  });
});
...
(0, _cucumber.Then)(/^the basket should show the configured extra matching the PDP selection$/, async function () {
  const {
    screen: {
      page
    },
  ...
  const namesSelector = (0, _webElementHelper.getElementLocator)(page, "basket line extra names", globalConfig);
  const pricesSelector = (0, _webElementHelper.getElementLocator)(page, "basket line extra prices", globalConfig);
  const names = await page.locator(namesSelector).allTextContents();
  const matchIndex = names.findIndex(n => n.includes(name));
  (0, _test.expect)(matchIndex, `Expected extra "${name}" not found among basket extras: ${names.join(", ")}`).toBeGreaterThanOrEqual(0);
  const prices = await page.locator(pricesSelector).allTextContents();
  const actualPriceDelta = parsePrice(prices[matchIndex]);
  (0, _test.expect)(Math.abs(actualPriceDelta - priceDelta)).toBeLessThanOrEqual(0.5);
});
...
(0, _cucumber.When)(/^I select a different finish option and validate the PDP price updates$/, async function () {
  const {
    screen: {
      page
    },
  ...
  const totalSelector = (0, _webElementHelper.getElementLocator)(page, "configurator total value", globalConfig);
  const optionsSelector = (0, _webElementHelper.getElementLocator)(page, "finish options", globalConfig);
  const options = page.locator(optionsSelector);
  await options.first().waitFor({
    state: "visible",
    timeout: 30000
  ...
  const parsedOptions = optionTexts.map(text => {
    ...
    return {
      price: lastPrice ? parsePrice(lastPrice) : NaN
    };
  });
  const targetIndex = parsedOptions.findIndex(_ref => {
    let {
      price
    } = _ref;
    return !isNaN(price) && Math.abs(price - totalBefore) > 0.01;
  });
  if (targetIndex === -1) {
  ...
  for (let attempt = 0; attempt < 2 && !matched; attempt++) {
    await options.nth(targetIndex).click();
    matched = await (0, _waitForBehaviour.waitFor)(async () => {
      const totalAfter = parsePrice(await page.textContent(totalSelector));
      return Math.abs(totalAfter - expectedTotal) < 0.02;
    }, {
      timeout: 8000,
  ...
  this.globalVariables["selected finish"] = JSON.stringify({
    name,
    price: expectedTotal
  });
});
(0, _cucumber.Then)(/^the basket should show the selected finish$/, async function () {
  const {
    screen: {
      page
    },
  ...
  const variantSelector = (0, _webElementHelper.getElementLocator)(page, "basket line variant", globalConfig);
  const variantText = await page.textContent(variantSelector);
  (0, _test.expect)(variantText, `Expected basket variant text to mention finish "${name}", got "${variantText}"`).toContain(name);
  const totalSelector = (0, _webElementHelper.getElementLocator)(page, "basket line total price", globalConfig);
  const actualTotal = parsePrice(await page.textContent(totalSelector));
  (0, _test.expect)(Math.abs(actualTotal - price)).toBeLessThanOrEqual(0.5);
});
...
(0, _cucumber.When)(/^I check the first optional extra and validate the PDP total updates$/, async function () {
  const {
    screen: {
      page
    },
  ...
  const totalSelector = (0, _webElementHelper.getElementLocator)(page, "configurator total value", globalConfig);
  const checkboxesSelector = (0, _webElementHelper.getElementLocator)(page, "optional extra checkboxes", globalConfig);
  ...
  const rowTexts = await checkboxes.evaluateAll(els => els.map(el => {
    let node = el;
...
(0, _cucumber.When)(/^I uncheck that optional extra and validate the PDP total reverts$/, async function () {
  const {
    screen: {
      page
    },
...
// line item + extras.
(0, _cucumber.Then)(/^the basket grand total should be internally consistent$/, async function () {
  const {
    screen: {
      page
    },
  ...
  const mainTotalSelector = (0, _webElementHelper.getElementLocator)(page, "basket line total price", globalConfig);
  const extrasPricesSelector = (0, _webElementHelper.getElementLocator)(page, "basket line extra prices", globalConfig);
  const grandTotalSelector = (0, _webElementHelper.getElementLocator)(page, "basket total", globalConfig);
  ...
  (0, _test.expect)(Math.abs(actualTotal - expectedTotal)).toBeLessThanOrEqual(0.02);
});