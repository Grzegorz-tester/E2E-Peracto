...
(0, _cucumber.When)(/^I switch to the "([^"]*)" quote tab$/, async function (tabKey) {
  const {
    screen: {
      page
    },
  ...
  const tabSelector = (0, _webElementHelper.getElementLocator)(page, tabKey, globalConfig);
...
(0, _cucumber.When)(/^I save the quote and wait for it to persist$/, async function () {
  const {
    screen: {
      page
    },
  ...
  const saveSelector = (0, _webElementHelper.getElementLocator)(page, "save button", globalConfig);
  const [response] = await Promise.all([page.waitForResponse(r => /staging-api\.[^/]+\/quotes\/[^/?]+$/.test(r.url()) && r.request().method() === "PUT", {
    timeout: 15000
  }), page.locator(saveSelector).click()]);
  (0, _test.expect)(response.status()).toBeLessThan(300);
});
...
(0, _cucumber.Given)(/^I duplicate this quote for a company matching "([^"]*)" and use the copy$/, async function (companyText) {
  const {
    screen: {
      page
    },
  ...
  const kebabSelector = (0, _webElementHelper.getElementLocator)(page, "quote kebab menu button", globalConfig);
  const companyInputSelector = (0, _webElementHelper.getElementLocator)(page, "duplicate quote company input", globalConfig);
  const confirmSelector = (0, _webElementHelper.getElementLocator)(page, "duplicate quote confirm button", globalConfig);
  await (0, _waitForBehaviour.waitFor)(async () => {
    ...
    try {
      ...
      await page.locator(companyInputSelector).click({
        timeout: 5000
      });
      await page.locator(".select-search-option", {
        hasText: companyText
      }).first().click({
        timeout: 5000
      });
      ...
      await page.waitForURL(url => url.toString() !== urlBefore && /\/account\/staff-quotes\/[^/]+$/.test(url.toString()), {
        timeout: 8000
      });
...
(0, _cucumber.When)(/^I create a new section named "([^"]*)"$/, async function (sectionName) {
  const {
    screen: {
      page
    },
  ...
  const nameInputSelector = (0, _webElementHelper.getElementLocator)(page, "create section name input", globalConfig);
  const createButtonSelector = (0, _webElementHelper.getElementLocator)(page, "create section button", globalConfig);
  await page.locator(nameInputSelector).fill(sectionName);
  await page.locator(createButtonSelector).click();
  await (0, _waitForBehaviour.waitFor)(() => page.locator(`:visible:text("${sectionName}")`).first().isVisible());
});
...
(0, _cucumber.When)(/^I add the catalogue item matching "([^"]*)" to the section I just created$/, async function (skuFragment) {
  const {
    screen: {
      page
    }
  ...
  await searchInput.type(skuFragment, {
    delay: 50
  });
...
(0, _cucumber.When)(/^I add a custom item named "([^"]*)" to the section I just created$/, async function (itemName) {
  const {
    screen: {
      page
    },
  ...
  const nameInputSelector = (0, _webElementHelper.getElementLocator)(page, "custom item name input", globalConfig);
  ...
  await (0, _waitForBehaviour.waitFor)(() => page.locator(`:visible:text("${itemName}")`).first().isVisible());
});
...
(0, _cucumber.Then)(/^the text "([^"]*)" should be displayed on the quote$/, async function (text) {
  const {
    screen: {
      page
    }
  } = this;
  await (0, _test.expect)(page.locator(`:visible:text("${text}")`).first()).toBeVisible({
    timeout: 15000
  });
});
...
const sectionContainer = (page, sectionName) => page.locator(":visible").filter({
  hasText: sectionName
}).filter({
  hasText: "Subtotal"
}).last();
...
// itself.
async function fillAndSubmitDiscountModal(page, globalConfig, discount) {
  const discountInputSelector = (0, _webElementHelper.getElementLocator)(page, "discount modal input", globalConfig);
  const submitSelector = (0, _webElementHelper.getElementLocator)(page, "discount modal submit button", globalConfig);
  await page.locator(discountInputSelector).fill(discount);
  await page.locator(submitSelector).click();
}
(0, _cucumber.When)(/^I apply an additional discount of "([^"]*)" to the whole quote$/, async function (discount) {
  const {
    screen: {
      page
    },
  ...
  const triggerSelector = (0, _webElementHelper.getElementLocator)(page, "apply additional discount button", globalConfig);
  await page.locator(triggerSelector).click();
  await fillAndSubmitDiscountModal(page, globalConfig, discount);
});
(0, _cucumber.When)(/^I apply a discount of "([^"]*)" to the "([^"]*)" section$/, async function (discount, sectionName) {
  const {
    screen: {
      page
    },
...
(0, _cucumber.Then)(/^the "([^"]*)" discount value should still read "([^"]*)"$/, async function (sectionName, expectedValue) {
  const {
    screen: {
      page
    }
  ...
  await (0, _test.expect)(discountInput).toHaveValue(expectedValue, {
    timeout: 10000
  });
  await page.keyboard.press("Escape");
});
(0, _cucumber.When)(/^I apply a discount of "([^"]*)" to the product named "([^"]*)"$/, async function (discount, productName) {
  const {
    screen: {
      page
    },
  ...
  const row = page.locator("tr:visible", {
    hasText: productName
  }).first();
  ...
  const discountInputSelector = (0, _webElementHelper.getElementLocator)(page, "product edit modal discount input", globalConfig);
  const closeSelector = (0, _webElementHelper.getElementLocator)(page, "product edit modal close button", globalConfig);
  await page.locator(discountInputSelector).fill(discount);
  await page.locator(discountInputSelector).press("Tab");
  await page.locator(closeSelector).click();
});
(0, _cucumber.When)(/^I duplicate the "([^"]*)" section$/, async function (sectionName) {
  const {
    screen: {
      page
    }
...
(0, _cucumber.When)(/^I delete the "([^"]*)" section$/, async function (sectionName) {
  const {
    screen: {
      page
    }
...
(0, _cucumber.When)(/^I duplicate the product named "([^"]*)"$/, async function (productName) {
  const {
    screen: {
      page
    }
...
(0, _cucumber.When)(/^I delete the product named "([^"]*)"$/, async function (productName) {
  const {
    screen: {
      page
    }
...
async function openQuoteKebabMenuItem(page, globalConfig, menuItemText) {
  const kebabSelector = (0, _webElementHelper.getElementLocator)(page, "quote kebab menu button", globalConfig);
  await page.locator(kebabSelector).first().click();
...
(0, _cucumber.When)(/^I add a note "([^"]*)" to the quote$/, async function (note) {
  const {
    screen: {
      page
    },
...
(0, _cucumber.When)(/^I add an internal note "([^"]*)" to the quote$/, async function (note) {
  const {
    screen: {
      page
    },
...
(0, _cucumber.When)(/^I add S43 project number "([^"]*)" to the quote$/, async function (projectNumber) {
  const {
    screen: {
      page
    },
...
// repeated runs.
(0, _cucumber.When)(/^I delete this quote$/, async function () {
  const {
    screen: {
      page
    },
